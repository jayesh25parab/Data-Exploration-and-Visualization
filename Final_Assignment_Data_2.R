...
####################################################################################################        
        output$DayPlot <- renderPlot({
          ...
          top_day <- ggplot(accident_day, aes(x = DAY_OF_WEEK, y = total_accident)) + 
            ...
            labs(x="Day of Week", y = "Total Accidents", title = "Accident per City") + 
            geom_text(aes(label = total_accident), vjust =  -.5) +
            theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
          print(top_day)
        })
...
####################################################################################################         
        output$HourPlot <- renderPlot({
          ...
          top_hour <- barplot(counts, main="Accidents",
                              xlab="Number of Accidents")
      ...
      output$GenderPlot <- renderPlot({
        ...
        output$InjuryPlot <- renderPlot({
          ...
          if (input$demographics_injury) {
            # Slices_1 <- c(sum(crashes_year$INJ_OR_FATAL),
            #             sum(crashes_year$NONINJURED))
            # lbls_1 <- c("Injured", "Not Injured")
            # pct_1 <- round(Slices_1/sum(Slices_1)*100)
            # lbls_1 <- paste(lbls_1, pct_1) # add percents to labels 
            # lbls_1 <- paste(lbls_1,"%",sep="") # ad % to labels 
            # injuryplot <-pie(Slices_1,labels = lbls_1, col=rainbow(length(lbls_1)),
            #         main="Pie Chart of Injury")
            # print(injuryplot)
        ...
        output$PeoplePlot <- renderPlot({
          ...
          if (input$demographics_people_type) {
            # Slices_3 <- c(sum(crashes_year$BICYCLIST), sum(crashes_year$PASSENGER),
            ...
            # lbls_3 <- c("Bicyclist", "Passenger","Driver","Pedestrian", "Pillion","Unknown","Motorist")
            ...
            # peopleplot <-pie(Slices_3,labels = lbls_3, col=rainbow(length(lbls_3)),
            #                      main="Pie Chart of People Type")
            # print(peopleplot)
        ...
        output$InjuryTypePlot <- renderPlot({
          ...
          if (input$demographics_injury_type) {
            # Slices_2 <- c(sum(crashes_year$FATALITY), sum(crashes_year$SERIOUSINJURY),
            #               sum(crashes_year$OTHERINJURY))
            # lbls_2 <- c("Fatality", "Serious","Not Serious")
            ...
            # injurytypeplot <-pie(Slices_2,labels = lbls_2, col=rainbow(length(lbls_2)),
        ...
        output$VehiclePlot <- renderPlot({
          ...
          if (input$demographics_vehicle_type) {
            # Slices_4 <- c(sum(crashes_year$PASSENGERVEHICLE), sum(crashes_year$MOTORCYCLE),
            ...
            # lbls_4 <- c("Passenger", "Motorcycle","Public","Heavy", "Bicycle","Unknown")
            ...
            # vehicleplot <-pie(Slices_4,labels = lbls_4, col=rainbow(length(lbls_4)),
      ...
      output$AnalysisPlot <- renderPlot({
        ...
        # ggplot(Analysis_data, aes(LGA_NAME, total_accident)) +
        #   geom_point(aes(shape = ALCOHOL_RELATED, color = HIT_RUN_FLAG, size = ALCOHOL_RELATED))
